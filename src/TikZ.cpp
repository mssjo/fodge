...
void _print_TikZ_header(std::ostream& tikz){
...
int Diagram::TikZ(const std::string& filename, const std::vector<Diagram>& diagrs, 
                  int split, double radius)
{
    ...
    if(split > 0){
        for(int n = 0; n * split < diagrs.size(); n++){
            std::string splitfile = filename + "_tikz_" + std::to_string(n+1) + ".tex";
            tikz.open(splitfile, std::ios::out | std::ios::trunc);
            if(tikz.fail()){
                std::cerr << "ERROR: could not open file \"" << splitfile << "\"\n";
                return 1;
            }
            _print_TikZ_header(tikz);
            tikz << "%%% This file is part " << (n+1) 
                 << " of " << (1 + (diagrs.size() / split)) << ".\n\n";
            
            std::cout << "TikZing diagrams to file \"" << splitfile << "\"...\n"; 
        ...
        tikz.open(filename + "_tikz.tex", std::ios::out | std::ios::trunc);
        if(tikz.fail()){
            std::cerr << "ERROR: could not open file \"" << filename << "_tikz.tex\"\n";
            return 1;
        }
        _print_TikZ_header(tikz);
        tikz << "\n";
        
        std::cout << "TikZing diagrams to file  \"" << filename << "_tikz.tex\"...\n";  
